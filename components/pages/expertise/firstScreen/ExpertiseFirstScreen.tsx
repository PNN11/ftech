...
import ContactUsBlock from '@/components/widgets/ContactUs'

const ExpertiseFirstScreen: FC = () => {
    const { t } = useTranslation()

    const { title, description, explore, service } = t('expertise:first_screen', { returnObjects: true }) as {
        title: string
        description: string
        explore: string
        service: { title: string; description: string; action_button: string }
    }
    return (
        <>
                        ...
                        >
                            {expertise.map(({ longDescription, href, number, title, image }) => (
                                <SwiperSlide key={title} className="relative overflow-hidden">
                                    ...
                                    />
                                    <div className="bg-expertise-first-screen absolute inset-0" />
                                </SwiperSlide>
                            ))}
                        </Swiper>
                    </div>
            ...
            <section className="grid grid-cols-1 gap-2 md:grid-cols-2 lg:grid-cols-[58%_1fr]">
                <ContactUsBlock />
                <div className="rounded-3xl border border-gray-200 bg-white-10 p-6 md:rounded-8 md:p-8">
                    <div className="max-w-58.75 space-y-3">
                        <Heading variant="h5">{service.title}</Heading>
                        <Paragraph variant="p1">{service.description}</Paragraph>
                        <Link className="inline-block" href="/services">
                            <Button variant="outlined">{service.action_button}</Button>
                        </Link>
                    </div>
                </div>
            </section>
